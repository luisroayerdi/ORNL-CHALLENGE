...
from qiskit.algorithms import QuantumPhaseEstimation
...
def hhl_solver(matrix, vector):
    """Custom HHL Solver using Quantum Phase Estimation."""
    num_qubits = int(np.ceil(np.log2(matrix.shape[0])))  # Number of qubits needed
    circuit = QuantumCircuit(num_qubits + 1)  # Extra qubit for measurement

    # Encode the matrix and vector into the quantum state
    for i in range(num_qubits):
        circuit.h(i)

    # Apply Quantum Phase Estimation
    qpe = QuantumPhaseEstimation(num_evaluation_qubits=num_qubits, unitary=QFT(num_qubits).to_instruction())
    circuit.append(qpe, range(num_qubits + 1))
    ...
    # Extract the solution from the statevector
    quantum_solution = np.real(statevector[:matrix.shape[0]])  # Take real part
...
if __name__ == '__main__':
    ...
    print(f'==================Solving with Custom HHL================', flush=True)
    
    t = time.time()
    quantum_solution = hhl_solver(matrix, vector)
    t_hhl = time.time() - t
